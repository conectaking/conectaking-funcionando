...
function parsePastedActivation(text) {
    ...
    if (n >= 2) {
        return {
            ...
            partial: true,
            sections,
            warning: 'Divisão parcial (' + n + ' blocos). Revise sentena e decreto se faltarem.'
        };
    }
    const found = Object.keys(sections).filter((k) => String(sections[k] || '').trim());
    return {
        error: 'Não foi possvel dividir. Encontrado: ' + (found.join(', ') || 'nada')
            + '. Use os marcadores: FUNDAMENTO SAGRADO, FRASES DE IMPACTO, SENTENA DE ATIVAO.'
    };
}
