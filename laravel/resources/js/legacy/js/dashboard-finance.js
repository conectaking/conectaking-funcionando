    ...
    try {
        ...
        window.renderUnifiedKingTab = function (tabId) {
            ...
            if (tabId === 'fluxo') {
                ...
                listBase.sort(function(a, b) { return (b.data || '').localeCompare(a.data || ''); });
                const list = listBase;
                const canEdit = function(f) { return !f._origem && (typeof f.id === 'number' || (f.id != null && String(f.id).match(/^[0-9]+$/))); };
                container.innerHTML = `
            ...
            window._kingFinanceOpenModal = function (modalType, pessoaIdOrEditItem) {
                ...
                if (modalType === 'fluxo') {
                    ...
                    if (modalType === 'trabalho' && window._kingFinanceEditItem) {
                        ...
                        // Em modo edição: ocultar campo entrada (entradas gerenciadas pelo botão dedicado)
                        var entradaWrap = formEl.querySelector('input[name="entrada"]'); if (entradaWrap) { entradaWrap.closest('label') && (entradaWrap.closest('label').style.display = 'none'); entradaWrap.style.display = 'none'; var prevLabel = entradaWrap.previousElementSibling; if (prevLabel && prevLabel.tagName === 'LABEL') prevLabel.style.display = 'none'; }
                    }
                ...
                formEl.onsubmit = async function (e) {
                    ...
                    if (!editItem) {
                        ...
                        if (mType === 'trabalho') {
                            ...
                            entry.dataPrevista = data.dataPrevista || '';
                            entry.pagamentos = [];
                            // Entrada inicial (AV/sinal) digitada no formulário
                            var entradaInicial = parseFloat(String(data.entrada || '0').replace(',', '.')) || 0;
                            if (entradaInicial > 0) {
                                var hoje = new Date(); var hh = String(hoje.getHours()).padStart(2,'0') + ':' + String(hoje.getMinutes()).padStart(2,'0');
                                entry.pagamentos.push({ valor: entradaInicial, data: hoje.toISOString().slice(0, 10), hora: hh });
                            }
                            delete entry.entrada;
                        }
...
window.openFinanceDetailModal = async function (type) {
    ...
    if (type === 'expense') {
        ...
        if (itensReceitas.length === 0) {
            ...
            // Incluir pagamentos de trabalhos do king_sync nas receitas do mês
            var kingDbModal = window._kingFinanceDb || {};
            (kingDbModal.trabalhos || []).forEach(function(t) {
                (t.pagamentos || []).forEach(function(p) {
                    var dt = (p.data || '').toString().trim().slice(0, 10);
                    if (!dt || dt.slice(0,7) !== mesRefInc) return;
                    itensReceitas.push({ origem: 'trabalho', descricao: t.descricao || t.servico || 'Trabalho', cliente: t.cliente || t.nome || null, valor: parseFloat(p.valor) || 0, data: dt });
                });
            });
        }
