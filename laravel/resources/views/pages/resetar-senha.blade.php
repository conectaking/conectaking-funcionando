    ...
    <script>
        (function() {
            ...
            form.addEventListener('submit', async function(e) {
                ...
                if (pwd.length < 6) { showMsg('A senha precisa ter no mínimo 6 caracteres.', true); return; }
                if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(pwd)) {
                    showMsg('A senha deve conter maiúscula, minúscula e número.', true);
                    return;
                }
                if (pwd !== pwdConfirm) { showMsg('As senhas não coincidem.', true); return; }
                btn.disabled = true;
                ...
                try {
                    ...
                    if (res.ok && data.success) {
                        ...
                        showMsg(data.message || 'Token inválido ou expirado. Solicite uma nova recuperação.', true);
                    }
                } catch (err) {
