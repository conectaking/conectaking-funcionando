...
<div id="cropper-modal" class="modal-overlay">
    ...
    <!-- Base da API (produção por padrão; local apenas quando explicitamente solicitado) -->
    <!-- Base da API: mesma origem no VPS/dominio; :5000 so em localhost -->
    <script>
      (function () {
        try {
          ...
          var isProdHost = host === 'conectaking.com.br' || host === 'www.conectaking.com.br' || host.endsWith('.conectaking.com.br');
          var wantLocal =
            (params.get('api') || '').toLowerCase() === 'local' ||
            (isLocalHost && (port === '5000' || localStorage.getItem('useLocalApi') === 'true'));
          ...
          var localApiBase = (isLocalHost && port === '5000') ? sameOrigin : ('http://' + (host || 'localhost') + ':5000');
          var prodApiBase = isProdHost ? sameOrigin : 'https://www.conectaking.com.br';
          window.API_BASE = wantLocal ? localApiBase : prodApiBase;
    ...
    <!-- Carrega api-config.js dinamicamente (evita document.write e aviso de parser-blocking) -->
    <script>
      (function(){ var b = (window.API_BASE || window.location.origin || 'https://www.conectaking.com.br').replace(/\/$/, ''); var s = document.createElement('script'); s.src = b + '/api-config.js'; s.async = true; document.body.appendChild(s); })();
    </script>
    <script src="js/module-link-limits.js?v=2026-01-28-03"></script>
