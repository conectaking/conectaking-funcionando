...
<head>
    ...
    <style id="checkout-critical-css">
        .checkout-icon { width: 1.25rem; height: 1.25rem; display: inline-block; vertical-align: middle; fill: currentColor; }
        .checkout-icon-lg { width: 1.5rem; height: 1.5rem; }
    ...
    <script>
        (function() {
            var c = { darkMode: 'class', theme: { extend: {
                colors: {
            ...
            function apply() {
                if (typeof window.tailwind !== 'undefined') { window.tailwind.config = c; return; }
                setTimeout(apply, 30);
            }
            apply();
        })();
...
<!-- Top Navigation Bar -->
<div class="checkout-header sticky top-0 z-50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md">
    <div class="checkout-header-inner flex items-center p-4 pb-2 justify-between max-w-md mx-auto">
        <a href="{{ $baseUrl }}/{{ $slug }}/form/{{ $itemId }}" class="text-slate-900 dark:text-white flex size-10 items-center justify-center cursor-pointer" aria-label="Voltar">
            <svg class="checkout-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
        </a>
        <h2 class="text-slate-900 dark:text-white text-lg font-light tracking-[0.2em] uppercase flex-1 text-center pr-10">Checkout</h2>
    </div>
    <!-- Progress Stepper -->
    <div class="checkout-stepper flex w-full max-w-xs mx-auto flex-row items-center justify-between py-6 relative">
        <div class="absolute top-1/2 left-0 w-full h-[1px] bg-slate-200 dark:bg-slate-800 -z-10 translate-y-[-50%]"></div>
        <div class="checkout-step flex flex-col items-center gap-2">
            <div class="checkout-step-dot h-2 w-2 rounded-full bg-slate-400 dark:bg-slate-600"></div>
            <span class="text-[10px] uppercase tracking-widest text-slate-400 dark:text-slate-600 font-medium">Resumo</span>
        </div>
        <div class="checkout-step current flex flex-col items-center gap-2">
            <div class="checkout-step-dot current h-3 w-3 rounded-full bg-primary ring-4 ring-primary/20"></div>
            <span class="text-[10px] uppercase tracking-widest text-primary font-bold">Pagamento</span>
        </div>
        <div class="checkout-step flex flex-col items-center gap-2">
            <div class="checkout-step-dot h-2 w-2 rounded-full bg-slate-200 dark:bg-slate-800"></div>
            <span class="text-[10px] uppercase tracking-widest text-slate-200 dark:text-slate-800 font-medium">Concluído</span>
        </div>
    </div>
</div>

<main class="checkout-main max-w-md mx-auto pb-32 px-6">
    <!-- Order Summary Card (igual ao luxury reference) -->
    <div class="px-0 py-4">
        <div class="checkout-card bg-white dark:bg-card-dark rounded-xl p-5 shadow-sm border border-slate-100 dark:border-white/5">
            <div class="checkout-card-inner flex items-center justify-between gap-4">
                <div class="checkout-card-text flex flex-col gap-1 flex-1 min-w-0">
                    <p class="checkout-card-label text-accent-gold text-[10px] uppercase tracking-[0.15em] font-semibold">Inscrição</p>
                    <p class="checkout-card-title text-slate-900 dark:text-white text-base font-medium leading-tight tracking-tight truncate">{{ $formTitle }}</p>
                    @if($responderName)<p class="checkout-card-sub text-slate-400 dark:text-slate-500 text-xs font-normal">{{ $responderName }}</p>@endif
                    <div class="mt-4">
                        <span class="checkout-card-price text-slate-900 dark:text-white text-lg font-bold tracking-tight" id="price-display">R$ {{ number_format($priceCents / 100, 2, ',', '') }}</span>
                    </div>
                ...
                <div class="checkout-card-img w-24 h-24 flex-shrink-0 bg-center bg-no-repeat bg-cover rounded-lg shadow-inner bg-slate-100 dark:bg-slate-800 overflow-hidden"@if($formCoverImageUrl) style="background-image: url('{{ $formCoverImageUrl }}');"@endif>
                    @if(! $formCoverImageUrl)<span class="w-full h-full flex items-center justify-center text-slate-500"><svg class="checkout-icon checkout-icon-lg" style="width:2.5rem;height:2.5rem" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></span>@endif
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="px-0 pt-6 pb-2">
        <h3 class="checkout-section-title text-slate-900 dark:text-white tracking-[0.05em] text-xl font-light">Método de pagamento</h3>
        <p class="checkout-section-sub text-slate-500 text-xs mt-1">Transações criptografadas e seguras.</p>
    </div>

    <!-- Pix vs Cartão (tabs estilo luxury) -->
    <div class="checkout-methods px-0 flex gap-3 mb-4">
        <div class="checkout-method selected method-card flex-1 rounded-xl p-4 border-2 border-slate-200 dark:border-slate-800 cursor-pointer transition-all" data-method="pix">
            <div class="flex items-center gap-2">
                <svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                <span class="font-semibold text-sm">Pix</span>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Instantâneo</p>
        </div>
        <div class="checkout-method method-card flex-1 rounded-xl p-4 border-2 border-slate-200 dark:border-slate-800 cursor-pointer transition-all" data-method="card">
            <div class="flex items-center gap-2">
                <svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg>
    ...
    <div id="card-section" class="px-6 space-y-4" style="display:none;">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="relative">
                ...
                <span class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"><svg class="checkout-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg></span>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-2" id="installments-wrap">
            ...
            <select id="card-installments" class="w-full rounded-lg text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 bg-white dark:bg-transparent focus:border-primary focus:ring-1 focus:ring-primary h-14 px-4 text-base font-light">
                @for($i = 1; $i <= 12; $i++)<option value="{{ $i }}">{{ $i }}x sem juros</option>@endfor
            </select>
        </div>
        <div class="flex gap-3 pt-2">
            <button type="button" id="btn-pay-credit" class="flex-1 rounded-xl bg-primary/20 dark:bg-primary/30 text-primary border-2 border-primary h-14 font-semibold text-sm flex items-center justify-center gap-2"><svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2"/><path d="M1 10h22"/></svg> Crédito</button>
            <button type="button" id="btn-pay-debit" class="flex-1 rounded-xl border-2 border-slate-200 dark:border-slate-700 h-14 font-semibold text-sm flex items-center justify-center gap-2"><svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 10v9h16v-9M4 10h16M4 10V7a2 2 0 012-2h12a2 2 0 012 2v3"/></svg> Débito</button>
        </div>
    </div>

    <!-- Order Totals (igual ao luxury: Subtotal, Insured Shipping Complimentary, Total Amount) -->
    <div class="checkout-totals px-0 mt-10 space-y-3">
        <div class="flex justify-between items-center text-sm">
            <span class="text-slate-500 font-light">Subtotal</span>
            <span class="text-slate-900 dark:text-white font-medium">R$ {{ number_format($priceCents / 100, 2, ',', '') }}</span>
        </div>
        <div class="flex justify-between items-center text-sm">
            <span class="text-slate-500 font-light">Envio</span>
            <span class="checkout-shipping text-accent-gold font-medium uppercase text-[10px] tracking-widest">Cortesia</span>
        </div>
        <div class="total-row pt-4 border-t border-slate-100 dark:border-slate-900 flex justify-between items-center">
            <span class="text-slate-900 dark:text-white text-base font-light tracking-wide">Total</span>
            <span class="text-slate-900 dark:text-white text-xl font-bold">R$ {{ number_format($priceCents / 100, 2, ',', '') }}</span>
        </div>
    </div>

    <!-- Trust Badges (3 itens como no luxury: 256-bit SSL, Secure Pay, Insured) -->
    <div class="checkout-trust px-0 mt-8 flex justify-center items-center gap-6 opacity-40 grayscale">
        <div class="flex items-center gap-1.5">
            <svg class="checkout-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
            <span class="text-[10px] uppercase tracking-widest font-bold">256-bit SSL</span>
        </div>
        <div class="flex items-center gap-1.5">
            <svg class="checkout-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
            <span class="text-[10px] uppercase tracking-widest font-bold">Pagamento seguro</span>
        </div>
        <div class="flex items-center gap-1.5">
            <svg class="checkout-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13"/><path d="M16 8h4l2 4-2 4h-4M5 16H1M2 8h2"/></svg>
...
<div id="cta-pix" class="checkout-cta fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background-light dark:from-background-dark via-background-light dark:via-background-dark to-transparent pt-10">
    <div class="max-w-md mx-auto">
        <button type="button" id="cta-btn-pix" class="checkout-cta-btn w-full bg-slate-900 dark:bg-white text-white dark:text-black h-16 rounded-xl font-semibold tracking-[0.2em] uppercase text-sm flex items-center justify-center gap-3 active:scale-[0.98] transition-all hover:ring-2 hover:ring-accent-gold shadow-2xl">
            <span>Gerar Pix</span>
            <svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
        </button>
...
<div id="cta-card" style="display:none;" class="checkout-cta fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background-light dark:from-background-dark via-background-light dark:via-background-dark to-transparent pt-10">
    <div class="max-w-md mx-auto">
        <button type="button" id="cta-btn-card" class="checkout-cta-btn w-full bg-slate-900 dark:bg-white text-white dark:text-black h-16 rounded-xl font-semibold tracking-[0.2em] uppercase text-sm flex items-center justify-center gap-3 active:scale-[0.98] transition-all hover:ring-2 hover:ring-accent-gold shadow-2xl">
            <span>Pagar agora</span>
            <svg class="checkout-icon checkout-icon-lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
        </button>
        <p class="text-center text-[10px] text-slate-400 dark:text-slate-600 mt-4 font-light tracking-wide uppercase">Devolução em 30 dias e suporte incluídos</p>
    </div>
</div>
