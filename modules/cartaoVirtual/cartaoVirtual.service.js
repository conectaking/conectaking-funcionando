...
/**
 * Retorna dados para renderizar a página do cartão público (GET /:identifier).
 * O controller é responsável por obter/release do client e enviar a resposta.
...
async function getProfilePageData(client, identifier, req) {
    ...
    logger.debug('🔍 Buscando perfil público', { identifier });
        ...
        if (userRes.rows.length === 0) {
            return { type: 'notFound', message: '<h1>404 - Perfil não encontrado</h1>' };
        }
        ...
        if (user.profile_slug && identifierLower === String(user.profile_slug).toLowerCase() && identifier !== user.profile_slug) {
            ...
            return { type: 'redirect', statusCode: 301, url: `/${user.profile_slug}${qs}` };
        }
        ...
        if (profileRes.rows.length === 0) {
            return { type: 'notFound', message: '<h1>404 - Perfil não configurado</h1>' };
        }
...
/**
 * Retorna dados JSON do perfil para a API GET /api/:identifier.
...
module.exports = { getProfilePageData, getProfileApi };
