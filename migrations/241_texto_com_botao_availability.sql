-- ===========================================
-- Migration 241 (parte 2): CHECK profile_items + module_plan_availability
-- Corre depois de 240_texto_com_botao_enum.sql
-- ===========================================
...
BEGIN
    ...
    RAISE NOTICE 'profile_items_item_type_check recriada com texto_com_botao.';
END $$;
...
BEGIN
    ...
    LOOP
        ...
        SELECT 'texto_com_botao', plan_code_var, true
        WHERE NOT EXISTS (
            SELECT 1 FROM module_plan_availability
            WHERE module_type = 'texto_com_botao' AND plan_code = plan_code_var
        );
    END LOOP;
END $$;

INSERT INTO module_plan_availability (module_type, plan_code, is_available)
SELECT 'texto_com_botao', sp.plan_code, true
FROM subscription_plans sp
WHERE sp.is_active = true
  AND NOT EXISTS (
      SELECT 1 FROM module_plan_availability mpa
      WHERE mpa.module_type = 'texto_com_botao' AND mpa.plan_code = sp.plan_code
  );

SELECT 'Migration 241: CHECK + module_plan_availability para texto_com_botao.' AS status;
