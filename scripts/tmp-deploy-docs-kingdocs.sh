...
set -e
cd /opt/conectaking
# ck-laravel.tgz MUST contain top-level dir `laravel/` (not flat files)
rm -rf laravel && tar xzf /tmp/ck-laravel.tgz
rm -f laravel/.env
# ck-middleware.tgz MUST contain `middleware/laravelProxy.js`
...
sleep 18
bash /tmp/tmp-smoke-docs-kingdocs.sh
