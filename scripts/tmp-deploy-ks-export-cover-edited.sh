...
echo '--- export ---'
curl -sS -D - -o /tmp/ex.json "http://127.0.0.1:5000/api/king-selection/galleries/${GID}/export" -H "$AUTH" \
  | tr -d '\r' | grep -iE 'HTTP/|x-conecta' | head -5
python3 -c "import json;o=json.load(open('/tmp/ex.json')); print('count',o.get('count'),'keys',sorted(o.keys())[:8]); assert o.get('success') and 'lightroom' in o"
...
curl -sS -D - -o /tmp/pw.json "http://127.0.0.1:5000/api/king-selection/galleries/${GID}/clients/${CID}/password" -H "$AUTH" \
  | tr -d '\r' | grep -iE 'HTTP/|x-conecta' | head -5
python3 -c "import json;o=json.load(open('/tmp/pw.json')); print({k:(('***' if k=='password' and v else v) for k,v in o.items())}); assert o.get('success') or 'Senha' in o.get('message','')"

echo '--- link-cover-preview ---'
...
python3 -c "import os;n=os.path.getsize('/tmp/lcp.bin'); print('bytes',n); assert n>20"
...
if [ -n "$PID" ] && [ "$PID" != "0" ]; then
  ...
  echo 'skip edited-upload (no photo)'
fi
echo DONE
