/**
 * UtilitÃ¡rios para envio de emails
 * Centraliza lÃ³gica de email
 */
...
function initEmailTransporter() {
    ...
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
        logger.warn('SMTP nÃ£o configurado: defina SMTP_USER e SMTP_PASS para envio de e-mails (recuperaÃ§Ã£o de senha, etc.).');
        return null;
    }
...
async function sendEmail(to, subject, html, text = null, attachments = []) {
    try {
        ...
        if (!emailTransporter) {
            logger.warn('Email transporter nÃ£o configurado');
        ...
        // Log detalhado para diagnÃ³stico (Gmail: Invalid login, ECONNREFUSED, etc.)
...
async function sendWelcomeEmail(userEmail, userName) {
    ...
    const html = `
        ...
        <p>Obrigado por se cadastrar. Seu cartÃ£o de visita digital estÃ¡ pronto para ser personalizado.</p>
        <p>Acesse seu painel para comeÃ§ar a criar seu perfil Ãºnico.</p>
...
async function sendPasswordResetEmail(userEmail, resetToken) {
    const baseUrl = config.urls.api || config.urls.frontend || 'https://www.conectaking.com.br';
    const resetUrl = `${baseUrl.replace(/\/$/, '')}/resetar-senha?token=${encodeURIComponent(resetToken)}`;
    const subject = 'RecuperaÃ§Ã£o de Senha - Conecta King';
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #1a1a1a; color: #ffffff;">
            <div style="text-align: center; margin-bottom: 30px;">
                ...
                <p style="color: #999;">RecuperaÃ§Ã£o de Senha</p>
            </div>
            
            <div style="background-color: #2a2a2a; padding: 30px; border-radius: 10px; margin-bottom: 20px;">
                <h2 style="color: #ffd700; margin-top: 0;">OlÃ¡!</h2>
                <p>VocÃª solicitou a recuperaÃ§Ã£o de senha para sua conta no Conecta King.</p>
            ...
            <div style="text-align: center; padding: 20px; border-top: 1px solid #333;">
                <p style="color: #999; font-size: 12px; margin: 0;">
                    â±ï¸ Este link expira em 1 hora.<br>
...
/**
 * Envia email de notificaÃ§Ã£o de expiraÃ§Ã£o de assinatura
 */
