...
router.get('/ping-version', (req, res) => res.json({
  success: true,
  version: '2026-05-19-r2-inventory',
  features: { r2Inventory: true, r2Cleanup: true },
  timestamp: new Date()
}));
...
function suggestWatermarkScaleForOutputDims(outW, outH, mode = 'fill') {
  ...
  if (ar > 1.35) s *= 1 + Math.min(0.12, (ar - 1.35) * 0.08);
  s = clamp(s, 0.15, 5.0);
...
async function loadWatermarkForGallery(pgClient, galleryId) {
  ...
  const logoOffsetX =
    rawOffX != null && Number.isFinite(parseFloat(rawOffX))
      ...
      : 0;
  const rawOffY = row.watermark_logo_offset_y;
  const logoOffsetY =
    rawOffY != null && Number.isFinite(parseFloat(rawOffY))
      ? Math.max(-50, Math.min(50, Math.round(parseFloat(rawOffY) * 100) / 100))
      : 0;
  const rawStW = row.watermark_stretch_w_pct;
  const rawStH = row.watermark_stretch_h_pct;
  const stretchWPct =
    rawStW != null && Number.isFinite(parseFloat(rawStW))
      ? Math.max(50, Math.min(400, Math.round(parseFloat(rawStW) * 100) / 100))
      : 100;
  const stretchHPct =
...
router.post('/galleries/:id/sales/clients/:clientId/round/:selectionBatch/payment-review', protectUser, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const curRow = (await dbClient.query(
      `SELECT status, amount_cents, note_admin${hasCum ? ', amount_received_cumulative_cents' : ''}${hasCourtesy ? ', courtesy_cents' : ''}${hasNegCol ? ', negotiated_total_cents' : ''}
       FROM king_client_payment_requests
       WHERE gallery_id=$1 AND client_id=$2 AND selection_batch=$3`,
      [galleryId, clientId, selectionBatch]
    )).rows?.[0];
...
router.get('/photos/:photoId/preview', protectUser, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const qOxp = parseFloat(String(req.query.wm_ox_p ?? req.query.wm_logo_offset_x_portrait ?? ''));
    if (Number.isFinite(qOxp)) wm.logoOffsetXPortrait = Math.max(-50, Math.min(50, Math.round(qOxp * 100) / 100));
    const qOyp = parseFloat(String(req.query.wm_oy_p ?? req.query.wm_logo_offset_y_portrait ?? ''));
    if (Number.isFinite(qOyp)) wm.logoOffsetYPortrait = Math.max(-50, Math.min(50, Math.round(qOyp * 100) / 100));
    const qOxl = parseFloat(String(req.query.wm_ox_l ?? req.query.wm_logo_offset_x_landscape ?? ''));
    if (Number.isFinite(qOxl)) wm.logoOffsetXLandscape = Math.max(-50, Math.min(50, Math.round(qOxl * 100) / 100));
    const qOyl = parseFloat(String(req.query.wm_oy_l ?? req.query.wm_logo_offset_y_landscape ?? ''));
    if (Number.isFinite(qOyl)) wm.logoOffsetYLandscape = Math.max(-50, Math.min(50, Math.round(qOyl * 100) / 100));
    ...
    const qStWp = parseFloat(String(req.query.wm_st_wp ?? req.query.wm_stretch_w_pct_portrait ?? ''));
    if (Number.isFinite(qStWp)) wm.stretchPortraitW = Math.max(50, Math.min(400, Math.round(qStWp * 100) / 100));
    const qStHp = parseFloat(String(req.query.wm_st_hp ?? req.query.wm_stretch_h_pct_portrait ?? ''));
    if (Number.isFinite(qStHp)) wm.stretchPortraitH = Math.max(50, Math.min(400, Math.round(qStHp * 100) / 100));
    const qStWl = parseFloat(String(req.query.wm_st_wl ?? req.query.wm_stretch_w_pct_landscape ?? ''));
    if (Number.isFinite(qStWl)) wm.stretchLandscapeW = Math.max(50, Math.min(400, Math.round(qStWl * 100) / 100));
    const qStHl = parseFloat(String(req.query.wm_st_hl ?? req.query.wm_stretch_h_pct_landscape ?? ''));
    if (Number.isFinite(qStHl)) wm.stretchLandscapeH = Math.max(50, Math.min(400, Math.round(qStHl * 100) / 100));
...
// R2 inventário/limpeza: routes/kingSelectionR2.routes.js (montado em server.js)
