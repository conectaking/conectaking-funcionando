...
// ============================================
// ROTAS ESPECÍFICAS PARA CADA TIPO DE MÓDULO
// ============================================
...
router.put('/items/banner/:id', protectUser, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Campos específicos do banner
        const bannerFields = ['title', 'destination_url', 'image_url', 'whatsapp_message', 'aspect_ratio', 'is_active', 'display_order'];
        const updateFields = [];
        const updateValues = [];
        let paramIndex = 1;
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ Banner ${itemId} atualizado com sucesso em ${duration}ms`);
...
// PUT /api/profile/items/link/:id - Atualizar link personalizado específico
router.put('/items/link/:id', protectUser, asyncHandler(async (req, res) => {
    const client = await db.pool.connect();
    const startTime = Date.now();
    try {
        ...
        console.log(`📝 PUT /api/profile/items/link/${itemId} - userId: ${userId}`);
        ...
        if (checkRes.rows.length === 0) {
            console.log(`❌ Link ${itemId} não encontrado ou não pertence ao usuário ${userId}`);
            return res.status(404).json({ message: 'Link não encontrado ou você não tem permissão para editá-lo.' });
        }

        // Campos específicos do link
        const linkFields = ['title', 'destination_url', 'image_url', 'icon_class', 'logo_size', 'is_active', 'display_order'];
        const updateFields = [];
        const updateValues = [];
        let paramIndex = 1;
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ Link ${itemId} atualizado com sucesso em ${duration}ms`);
...
router.put('/items/carousel/:id', protectUser, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Campos específicos do carousel
        const carouselFields = ['title', 'destination_url', 'image_url', 'aspect_ratio', 'is_active', 'display_order'];
        const updateFields = [];
        const updateValues = [];
        let paramIndex = 1;

        for (const field of carouselFields) {
            if (updates.hasOwnProperty(field)) {
                updateFields.push(`${field} = $${paramIndex++}`);
                updateValues.push(updates[field]);
            }
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ Carousel ${itemId} atualizado com sucesso em ${duration}ms`);
...
router.put('/items/pix/:id', protectUser, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Campos específicos do PIX
        const pixFields = ['title', 'pix_key', 'recipient_name', 'pix_amount', 'pix_description', 'icon_class', 'is_active', 'display_order'];
        const updateFields = [];
        const updateValues = [];
        let paramIndex = 1;
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ PIX ${itemId} atualizado com sucesso em ${duration}ms`);
...
router.put('/items/pdf/:id', protectUser, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Campos específicos do PDF
        const pdfFields = ['title', 'pdf_url', 'destination_url', 'is_active', 'display_order'];
        const updateFields = [];
        const updateValues = [];
        let paramIndex = 1;
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ PDF ${itemId} atualizado com sucesso em ${duration}ms`);
