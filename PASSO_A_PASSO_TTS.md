...
- **Erro "EPROTO" / "ssl3 alert handshake failure" nos logs (TTS):** é falha de **conexão TLS/rede** entre o servidor (ex.: Render) e os servidores do Google — **não é credencial errada**. Credencial GCP errada daria 401/403 ou "invalid_grant". Para confirmar que as credenciais estão certas: rode o backend **localmente** (`npm start`) com o mesmo `.env` e teste "Ouvir (áudio)" em `http://localhost:5000/...` ou chame `GET http://localhost:5000/api/bible/tts/audio?ref=Jo%203:16&version=nvi`. Se funcionar local e falhar no Render, o problema é o ambiente/rede do Render (TLS). Use **GCP_TTS_USE_REST=1** e Node 18+ no Render; se continuar, pode ser restrição de rede do provedor.
