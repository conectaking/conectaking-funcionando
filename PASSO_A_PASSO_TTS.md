...
- **Script tts.js com MIME type text/html:** abra a página com URL que inclua o diretório (ex.: `.../public_html/bible.html`); o script está como `./js/tts.js`.
...
- **Erro "EPROTO" / "ssl3 alert handshake failure" nos logs (TTS):** é falha de **conexão TLS/rede** entre o servidor (ex.: Render) e os servidores do Google — **não é credencial errada**. O código já faz o seguinte no Render: (1) usa **sempre API REST** (não gRPC) quando a variável de ambiente **RENDER** é `true`; (2) não usa agente HTTPS customizado, para evitar handshake failure. Confira no Render em **Environment** que **RENDER** está definido (o Render define automaticamente) e, se quiser forçar REST em qualquer host, use **GCP_TTS_USE_REST=1**. Node 18+ no Render é recomendado. Se o erro continuar, teste o mesmo `.env` **localmente** (`npm start`); se funcionar em local e falhar no Render, pode ser restrição de rede do provedor.
